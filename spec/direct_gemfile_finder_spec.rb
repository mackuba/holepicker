...
describe HolePicker::DirectGemfileFinder do
  let(:finder) { HolePicker::FileFinder }
  let(:path) { '/etc' }
  ...
  describe "#find_gemfiles" do
    ...
    it "should find gemfiles in given path" do
      finder.expects(:find_files).with(path, "\\( SKIP \\) -prune -or -name 'Gemfile.lock' -print").returns(['1', '2'])
    ...
    context "if passed path is relative" do
      it "should expand it" do
        finder.expects(:find_files).with('/usr', anything)
    ...
    context "if :only_current => true is passed" do
      subject { HolePicker::DirectGemfileFinder.new(:only_current => true) }

      it "should include only gemfiles in 'current'" do
        finder.expects(:find_files).with(path, "\\( SKIP \\) -prune -or -path '*/current/Gemfile.lock' -print")
    ...
    context "if :skip_ignored => false is passed" do
      subject { HolePicker::DirectGemfileFinder.new(:skip_ignored => false) }

      it "should not skip files that would normally be ignored" do
        finder.expects(:find_files).with(path, "-name 'Gemfile.lock'")
  ...
  describe "#skipped_directories" do
    before { @skipped = replace_const(HolePicker::DirectGemfileFinder, 'SKIPPED_DIRECTORIES', ['a', 'b', 'c']) }

    it "should return list of directory patterns joined with '-or'" do
      subject.send(:skipped_directories).should == 'a -or b -or c'
    end

    after { replace_const(HolePicker::DirectGemfileFinder, 'SKIPPED_DIRECTORIES', @skipped) }
  end
end
