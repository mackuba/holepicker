...
describe HolePicker::ConfigGemfileFinder do
  describe "#find_gemfiles" do
    ...
    it "should find config files in given path" do
      HolePicker::FileFinder.expects(:find_files).with('/etc', '-type f -or -type l').returns(paths)

      subject.find_gemfiles('/etc')
    end

    it "should calculate absolute path if path is relative" do
      HolePicker::FileFinder.expects(:find_files).with('/var', '-type f -or -type l').returns(paths)
    ...
    context "if config files are found" do
      ...
      it "should run use ConfigReaders to find gemfiles in each found config" do
        ...
        HolePicker::ConfigReader.expects(:new).with(paths[0]).returns(reader1)
        HolePicker::ConfigReader.expects(:new).with(paths[1]).returns(reader2)
    ...
    context "if some of the returned config files don't exist" do
      ...
      it "should ignore not existing configs" do
        HolePicker::FileFinder.stubs(:find_files).returns(paths)
        HolePicker::ConfigReader.expects(:new).with(paths[0]).never
        HolePicker::ConfigReader.expects(:new).with(paths[1]).returns(reader1)
