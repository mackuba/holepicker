...
describe HolePicker::Database do
  let(:version) { '1.2.4' }
  let(:vulnerabilities) {[
  ...
  let(:db) { HolePicker::Database.new(json) }
  ...
  describe "#initialize" do
    ...
    it "should extract vulnerabilities" do
      ...
      db.vulnerabilities.each { |v| v.should be_a(HolePicker::Vulnerability) }
    end
  ...
  describe ".load_from_json_file" do
    it "should build a database from a JSON string" do
      db = HolePicker::Database.load_from_json_file(json.to_json)

      db.should be_a(HolePicker::Database)
      db.vulnerabilities.length.should == 2
    end
  end
end
